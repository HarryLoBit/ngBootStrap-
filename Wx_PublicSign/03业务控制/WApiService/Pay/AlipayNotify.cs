...
namespace Com.Alipay
{
    ...
    public class Notify
    {
        ...
        private bool GetSignVeryfy(SortedDictionary<string, string> inputPara, string sign)
        {
            ...
            //获取待签名字符串
            string preSignStr = Core.CreateLinkString(sPara);
            preSignStr = preSignStr.Replace("¬", "&not");
            //获得签名验证结果
            bool isSgin = false;
            if (sign != null && sign != "")
            {
                ...
                {
                    case "RSA":
                        //isSgin = Aop.Api.Util.AlipaySignature.RSACheckV1(inputPara,_public_key,_input_charset);
                        isSgin = RSAFromPkcs8.Verify(preSignStr, sign, _public_key, _input_charset);
                        break;
                    case "MD5":
                        isSgin = AlipayMD5.Verify("", sign, _public_key, _input_charset);
                        break;
                    default:
                        break;
                }
